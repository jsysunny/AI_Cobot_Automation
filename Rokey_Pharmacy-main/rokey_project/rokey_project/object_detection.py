'''카메라를 열고 객체 인식을 진행하고, 객체의 바운딩 박스를 그려주는 파일'''
...
weights = os.path.join(package_share_directory, 'weights', 'shelf.pt')
model = YOLO(weights)
...
while True:
    ...
    for box in filtered_boxes:
        ...
        color = class_colors.get(cls, (0, 255, 0))  # 기본색: 초록

        cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), color, 2)
        cv2.putText(annotated_frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

    # 화면 출력
    cv2.imshow(f"YOLOv11 Webcam Detection (Conf >= {CONFIDENCE})", annotated_frame)
